...
import { token } from '../helper/axiosHelper';
...
const RequestedAttendance = () => {
  ...
  const getData = async () => {
    try {
      const response = axios.get(`https://fe5c-111-88-41-105.ngrok-free.app/api/login`, {
        headers: {
          'token': token
        }
      })
      ...
      const myData = await response.json().data.data.list;
      setAttData(myData);
      setIsLoaded(false);
